...
完成 `05_自主修改练习.ipynb` 末尾的“模型组合小实验”：保留基线、一个模型因素、一个训练因素及二者组合，共 4 组结果；前三组使用 Validation Accuracy 比较，组合确定后只报告一次 Test Accuracy，并提交训练曲线、Attention 图和 250–400 字报告。作业不按 Accuracy 高低排名。`06_注意力寻宝项目.ipynb` 保留为拓展挑战。
