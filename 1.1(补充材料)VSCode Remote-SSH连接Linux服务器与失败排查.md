...
先完成 [1.2：第一次代码课软件准备——实验室电脑与个人笔记本](./1.2%28补充材料%29第一次代码课软件准备-实验室电脑与个人笔记本.md)。`1.2` 是以下内容的唯一主教程：

- 实验室电脑和个人笔记本分别使用哪个 VS Code；
- `Remote - SSH` 等扩展如何安装；
- 如何检查 Windows OpenSSH Client。

本教程不再重复这些安装步骤，而是从“软件已经准备好”开始，专门讲 SSH 配置、首次连接和故障排查。如果 `1.2` 中任一检查没有通过，先返回 `1.2` 解决，再继续第 3 节。
...
#### 方案 A：使用课程统一测试的 VS Code 1.98.2（当前课堂方案）

当前课程服务器是旧版 Linux。个人笔记本按 [1.2：第一次代码课软件准备](./1.2%28补充材料%29第一次代码课软件准备-实验室电脑与个人笔记本.md) 使用班长发放的 `VSCode-win32-x64-1.98.2.zip`：解压到不含中文的目录后直接运行，不要与平时自动更新的 VS Code 混用。

这是针对当前课程服务器实测通过的兼容路线。它解决的是“本学期如何正常上课”，不是让所有旧版 VS Code 都变成推荐版本，也不改变服务器运行库较旧这一根本事实。

#### 方案 B：由管理员提供受支持的 Linux 环境（长期方案）

由服务器管理员升级或迁移到符合要求的操作系统，例如 CentOS/RHEL 8+、Debian 10+或 Ubuntu 20.04+，并同时核验内核、glibc 和 libstdc++。也可以为课程分配满足要求的新虚拟机或计算节点。

这是长期批量使用时最稳定的方案。完成迁移后，客户端可以恢复使用受支持的新版 VS Code，不必长期保留课程兼容版。

#### 方案 C：由管理员配置自定义 sysroot（技术性临时方案）
...
- 不要从网上随意寻找更老的 VS Code 版本反复试错。课堂只使用已经测试过的 `1.98.2` 课程副本，并与日常使用的新版 VS Code 分开保存。

### 9.7 本场景结论

客户端 SSH 配置、网络、主机指纹、密码验证、本地下载、SCP 上传和远端解压均已通过。新版 VS Code 的明确阻断条件是服务器操作系统运行库低于当前 VS Code Server 的最低要求，不能通过继续修改 SSH config 解决。当前课堂使用实测兼容的 VS Code 1.98.2；长期仍应由服务器维护方升级运行环境。
...
3. 新版 VS Code 已完成密码验证、本地下载、SCP 上传和远端解压，但服务器的 glibc 2.17 与旧 libstdc++ 不满足新版 VS Code Server 的最低要求。当前课堂使用实测兼容的 VS Code 1.98.2；长期需要管理员提供受支持的系统环境或实施受控的 sysroot workaround。
