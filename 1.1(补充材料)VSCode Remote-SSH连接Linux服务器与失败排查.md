...
| SSH 端口 | `22` | SSH 服务端口 |
| Linux 用户名 | `<你的 Linux 用户名>` | 登录 Linux 时使用自己分配到的账号 |
...
`172.23.168.7` 是内网地址。使用的电脑需要处于能够访问该地址的网络环境中，例如指定校园网络或学校提供的 VPN；具体接入方式以学校网络要求为准。
...
- `Port 22`：单独指定 SSH 端口。
- `User your_username`：指定 Linux 登录用户名。必须将 `your_username` 替换为分配给你的用户名。
...
4. 出现 `your_username@172.23.168.7's password:` 后输入你自己的账号密码。
...
- 成功进入 `[your_username@ag-g-02 ~]$`：账号与服务器正常，继续排查 VS Code。
